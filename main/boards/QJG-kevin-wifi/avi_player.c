...
void play_file(char *Filename)
{
    ...
    uint8_t fps = get_fps_avifle();
	uint8_t fps_delayTime = 1000 / fps;
    ESP_LOGI(TAG,"fps_delayTime:%d",fps_delayTime);
    ...
    { // 播放循环
        ...
        { // 显示帧
            ...
            jpeg_error_t ret;
            ret = esp_jpeg_decoder_one_image(pbuffer, Strsize, img_rgb565); // 使用乐鑫adf的jpg解码 速度快三倍
            if (ret != 0)
            {
                continue;
            }
            ...
            //执行代码耗费的时间
			uint32_t time = (uint32_t)((esp_timer_get_time() - fr_end) / 1000);//ms
            // ESP_LOGI(TAG, " time %d", time);
			if(time<fps_delayTime)
			{
				vTaskDelay(pdMS_TO_TICKS(fps_delayTime-time+10));
				
				// fps_update(fps);
			}else{
			    // ESP_LOGI(TAG, " FPS %u", 1000 / time);
				// fps_update(1000 / time);
			}
...
void IN_play(char *file, uint8_t state)
{
    ...
    case FACE_STATIC:
        ...
        case 1:
            ...
            memset(file_run_name, 0, sizeof(file_run_name));
            memset(file_out_name, 0, sizeof(file_out_name));
            memcpy(file_in_name, "/sdcard/Emoji1/sorry/sorry1.avi", strlen("/sdcard/Emoji1/sorry/sorry1.avi"));
            memcpy(file_run_name, "/sdcard/Emoji1/sorry/sorry2.avi", strlen("/sdcard/Emoji1/sorry/sorry2.avi"));
            memcpy(file_out_name, "/sdcard/Emoji1/sorry/sorry3.avi", strlen("/sdcard/Emoji1/sorry/sorry3.avi"));
            break;
        case 2:
    ...
    case FACE_HAPPY:
        ...
        memset(file_run_name, 0, sizeof(file_run_name));
        memset(file_out_name, 0, sizeof(file_out_name));
        memcpy(file_in_name, "/sdcard/Emoji1/happy/happy1.avi", strlen("/sdcard/happy/happy_1.avi"));
        memcpy(file_run_name, "/sdcard/Emoji1/happy/happy2.avi", strlen("/sdcard/Emoji1/happy/happy2.avi"));
        memcpy(file_out_name, "/sdcard/Emoji1/happy/happy3.avi", strlen("/sdcard/Emoji1/happy/happy3.avi"));
        break;
    ...
    case FACE_ANGRY:
        ...
        memset(file_run_name, 0, sizeof(file_run_name));
        memset(file_out_name, 0, sizeof(file_out_name));
        memcpy(file_in_name, "/sdcard/Emoji1/angry/angry1.avi", strlen("/sdcard/Emoji1/angry/angry1.avi"));
        memcpy(file_run_name, "/sdcard/Emoji1/angry/angry2.avi", strlen("/sdcard/Emoji1/angry/angry2.avi"));
        memcpy(file_out_name, "/sdcard/Emoji1/angry/angry3.avi", strlen("/sdcard/Emoji1/angry/angry3.avi"));
        break;
...
void Avi_Player_Task(void *arg)
{
    ...
    img_rgb565 = heap_caps_malloc(240 * 320 * 2, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM); // MALLOC_CAP_SPIRAM
    if (NULL == img_rgb565)
    {
