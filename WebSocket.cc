...
void copy_and_print_array_data(const uint8_t *source, size_t length) {  
    // 分配目标数组，长度与源数据长度相同  
    uint8_t *destination = (uint8_t *)malloc(length * sizeof(uint8_t));  
    if (destination == nullptr) {  
        ESP_LOGE(TAG, "Failed to allocate memory for destination array");  
        return;  
    }  
    ...
    // 打印数据  
    char buffer[100];  // 定义一个缓冲区来存储字符串  
    buffer[0] = '\0';  // 初始化缓冲区为空字符串  
    for (size_t i = 0; i < length; i++) {  
        char temp[4];  // 每个字节转换为十六进制字符串需要3个字符+1个终止符  
        snprintf(temp, sizeof(temp), "%02X ", destination[i]);  
...
void WebSocket::ReceiveTask() {
    ...
    while (transport_->connected()) {
        ...
        // copy_and_print_array_data((const uint8_t *)buffer[ret],buffer_offset);
        if (ret > 0) {
